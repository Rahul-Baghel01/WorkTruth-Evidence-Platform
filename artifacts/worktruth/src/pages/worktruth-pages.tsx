...
import type { CrossProjectVisualMatch, EvidenceImage, ManagedUser, Project, ProjectPriority, Role, UploadResult, VisualAnalysis } from '@workspace/api-client-react';
import {
  ActivityList,
  Card,
  CostBenchmarkBars,
...
function DonutChart({ points }: { points?: Array<{ label: string; value: number }> }) {
  ...
  if (total === 0) return <EmptyState icon={Layers} title="No projects yet" description="The priority mix will appear when projects are added." />;
    ...
    <main>
      <section className="landing-hero">
        ...
        <div className="hero-console">
          ...
          <div className="console-body"><div className="console-head"><div><span className="console-kicker">ILLUSTRATIVE PORTFOLIO SCALE</span><strong>District portfolio</strong></div><DataStamp>Illustrative</DataStamp></div><div className="console-score"><div><span>Projects</span><strong>184</strong><small>Illustrative, not live</small></div><div className="score-sweep"><div className="sweep-ring"><span>82</span><small>evidence<br />health</small></div></div></div><div className="console-lines"><div><span>Verification priorities</span><b className="text-danger">24</b><i style={{ width: '34%' }} /></div><div><span>Evidence within norm</span><b>137</b><i style={{ width: '74%' }} /></div><div><span>Recently resolved</span><b className="text-green">23</b><i style={{ width: '21%' }} /></div></div><div className="console-foot"><span><CircleDot size={12} /> Illustrative values for demonstration</span><span>Not live data</span></div></div>
...
export function ProjectsPage() {
  const [, setLocation] = useLocation();
  const sessionQuery = useGetSession({ query: { queryKey: getGetSessionQueryKey() } });
  const readOnly = isReadOnlyRole(sessionQuery.data?.user.role);
  ...
  const [form, setForm] = useState({ id: '', name: '', category: 'Roads', district: '', state: 'Uttar Pradesh', location: '', sanctionAmount: '', expenditure: '', progress: '', latitude: '', longitude: '', description: '' });
...
export function ProjectDetailPage() {
  ...
  const sessionQuery = useGetSession({ query: { queryKey: getGetSessionQueryKey() } });
  // Hides controls a read-only session cannot use. Not the access control:
  // the API refuses these writes for read-only roles regardless of the UI.
  const readOnly = isReadOnlyRole(sessionQuery.data?.user.role);
  ...
  if (projectQuery.isLoading) return <ShellPage><div className="detail-loading"><Skeleton className="h-8 w-48" /><Skeleton className="h-24" /><div className="detail-grid detail-top-grid"><Skeleton className="h-96" /><Skeleton className="h-96" /></div></div></ShellPage>;
...
function EvidenceLens({ icon: Icon, label, status, score, summary, children }: { icon: typeof BarChart2; label: string; status?: string; score?: number | null; summary?: string; children: React.ReactNode }) {
  ...
  return <Card className="evidence-lens"><div className="lens-head"><div className="lens-icon"><Icon size={17} /></div><div><span className="lens-label">{label}</span><StatusPill status={status || 'Pending Review'} /></div><span className="lens-score" title="This lens's own anomaly score (0% = no anomaly). Its contribution to the fused signal is shown separately.">{percentage === null ? '—' : `${Math.round(percentage)}%`}<small>anomaly score</small></span></div><p className="lens-explanation">{summary || 'Awaiting analysis for this evidence lens.'}</p>{children}</Card>;
}
...
function EvidenceImageGallery({ projectId, visual, readOnly }: { projectId: string; visual?: VisualAnalysis; readOnly: boolean }) {
  const queryClient = useQueryClient();
  ...
  return (
    <Card className="evidence-gallery-card">
      <SectionHeading eyebrow="VISUAL EVIDENCE" title="Evidence photographs" detail="Officer uploads use file metadata; demo image GPS and dates are seed-authored." />
      {!readOnly && <div className="gallery-upload">
          ...
          {images.map((image) => (
            <div className="gallery-item" key={image.id} data-testid={`evidence-image-${image.id}`}>
              <div className="gallery-thumb">
                <img src={evidenceImageSrc(projectId, image, getGetProjectImageFileUrl)} alt={image.label || image.originalFilename || `Evidence ${image.id}`} />
...
function ProgressRecordsCard({ projectId, readOnly }: { projectId: string; readOnly: boolean }) {
  const queryClient = useQueryClient();
  ...
  return (
    <Card className="progress-records-card">
      ...
      {!readOnly && <div className="progress-form">
...
export function UploadPage() {
  ...
  const sessionQuery = useGetSession({ query: { queryKey: getGetSessionQueryKey() } });
  if (isReadOnlyRole(sessionQuery.data?.user.role)) {
    return <ShellPage><PageIntro eyebrow="IMPORT REGISTER" title="Officer access required." description="Register imports change project records. Sign in with an officer account to import a CSV or XLSX register." /></ShellPage>;
  }
...
export function SettingsPage() {
  ...
  const user = sessionQuery.data?.user;
  return <ShellPage>
    <PageIntro eyebrow="METHOD & ACCESS" title="A clear method, by design." description="See how WorkTruth prioritises human verification and which access roles are available." />
    <div className="settings-layout">
      <div className="settings-main">
        <Card><SectionHeading eyebrow="VERIFICATION METHOD" title="Evidence fusion, with human judgement." detail="The system routes attention; it does not certify a project." /><div className="method-rows"><MethodRow number="01" title="Signals stay separate" description="Financial, visual, geographic, temporal, and text checks are calculated independently before they are combined." /><MethodRow number="02" title="Weights are visible" description="Each lens's confidence-adjusted contribution is shown on the project page." /><MethodRow number="03" title="The officer decides" description="A flagged project is a prompt for verification, never a conclusion about delivery or intent." /></div></Card>
        <Card><SectionHeading eyebrow="ACCESS MODEL" title="Current roles" detail="Permissions are enforced by the API." /><div className="method-rows"><MethodRow number="01" title="Available now" description="ADMIN manages users; OFFICER and VERIFIER can review records; VIEWER and GUEST are read-only." /><MethodRow number="02" title="Phase 2 / Future Scope" description="Jurisdiction-scoped roles (MP, state and ministry views) are planned, not implemented." /></div></Card>
      </div>
      <div className="settings-side"><Card className="account-card"><div className="settings-avatar">{initials(user?.name)}</div><div className="eyebrow">SIGNED IN AS</div><h2>{user?.name ?? 'Loading…'}</h2><p>{user?.role ?? ''}</p></Card><Card className="privacy-card"><LockKeyhole size={17} /><div><strong>Protected workspace</strong><span>Sessions and investigation updates are recorded for accountability.</span></div></Card></div>
    </div>
  </ShellPage>;
}
...
// GUEST is deliberately absent from the assignable options: it exists only for
// the shared "Explore demo" account, and the API rejects any attempt to assign
// it. It still needs a label and description because that account appears in
// this list, where an administrator can disable it to switch the demo off.
const ROLE_OPTIONS: Role[] = ['ADMIN', 'OFFICER', 'VERIFIER', 'VIEWER'];
const ROLE_LABEL: Record<Role, string> = { ADMIN: 'Administrator', OFFICER: 'Officer', VERIFIER: 'Verifier', VIEWER: 'Viewer', GUEST: 'Guest demo' };
const ROLE_DESCRIPTION: Record<Role, string> = {
  ...
  GUEST: 'Shared read-only demo account. Disable it to turn "Explore demo" off.',
};
...
function UserManagementRow({ user, pending, onToggleStatus, onChangeRole }: { user: ManagedUser; pending: boolean; onToggleStatus: () => void; onChangeRole: (role: Role) => void }) {
  return <div className="users-row" data-testid={`row-user-${user.id}`}>
    ...
    {/* The shared demo account's role is fixed — the API refuses to assign or
        reassign GUEST — so it is shown, not offered as a choice. It can still
        be disabled with the button in this row, which turns the demo off. */}
    {user.role === 'GUEST'
      ? <span className="users-role-fixed" data-testid={`role-fixed-${user.id}`} title={ROLE_DESCRIPTION.GUEST}>{ROLE_LABEL.GUEST}</span>
